...
public class AttachmentController {
 ...
 public AttachmentResponse uploadTrip(@PathVariable UUID tripId,@RequestPart("file") MultipartFile file) throws IOException {UUID agency=TenantContext.requireAgencyId();Trip trip=trips.findByIdAndAgency_Id(tripId,agency).orElseThrow(()->new ResourceNotFoundException("viagem não encontrada"));validate(file);AgencyAttachment saved=attachments.save(AgencyAttachment.builder().agency(agencies.getReferenceById(agency)).trip(trip).originalFilename(safeName(file.getOriginalFilename())).contentType(file.getContentType()).byteSize(file.getSize()).content(file.getBytes()).build());return AttachmentResponse.from(saved);}
