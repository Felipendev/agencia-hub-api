...
@RestController @RequestMapping("/attachments") @PreAuthorize("hasAnyRole('AGENCY_OWNER','SALES_AGENT')") @RequiredArgsConstructor
public class AttachmentController {
 ...
 private final AgencyAttachmentRepository attachments; private final SaleRepository sales; private final FinancialEntryRepository financialEntries; private final TripRepository trips; private final AgencyRepository agencies;
 ...
 @PostMapping(value="/trips/{tripId}",consumes=MediaType.MULTIPART_FORM_DATA_VALUE) @ResponseStatus(HttpStatus.CREATED)
 public AttachmentResponse uploadTrip(@PathVariable UUID tripId,@RequestPart("file") MultipartFile file,@RequestPart(value="description",required=false) String description) throws IOException {UUID agency=TenantContext.requireAgencyId();Trip trip=trips.findByIdAndAgency_Id(tripId,agency).orElseThrow(()->new ResourceNotFoundException("viagem não encontrada"));validate(file);AgencyAttachment saved=attachments.save(AgencyAttachment.builder().agency(agencies.getReferenceById(agency)).trip(trip).originalFilename(safeName(file.getOriginalFilename())).contentType(file.getContentType()).byteSize(file.getSize()).description(safeDescription(description)).content(file.getBytes()).build());return AttachmentResponse.from(saved);}
 ...
 @GetMapping("/trips/{tripId}") public java.util.List<AttachmentResponse> listTrip(@PathVariable UUID tripId){UUID agency=TenantContext.requireAgencyId();trips.findByIdAndAgency_Id(tripId,agency).orElseThrow(()->new ResourceNotFoundException("viagem não encontrada"));return attachments.findByTrip_IdAndAgency_IdOrderByCreatedAtDesc(tripId,agency).stream().map(AttachmentResponse::from).toList();}
 ...
 @PatchMapping("/{id}") public AttachmentResponse updateDescription(@PathVariable UUID id,@RequestBody UpdateAttachmentRequest request){AgencyAttachment attachment=attachments.findByIdAndAgency_Id(id,TenantContext.requireAgencyId()).orElseThrow(()->new ResourceNotFoundException("anexo não encontrado"));attachment.setDescription(safeDescription(request.description()));return AttachmentResponse.from(attachments.save(attachment));}
 @DeleteMapping("/{id}") @ResponseStatus(HttpStatus.NO_CONTENT) public void delete(@PathVariable UUID id){AgencyAttachment attachment=attachments.findByIdAndAgency_Id(id,TenantContext.requireAgencyId()).orElseThrow(()->new ResourceNotFoundException("anexo não encontrado"));attachments.delete(attachment);}
 ...
 private static String safeDescription(String description){return description==null?"":description.strip().substring(0,Math.min(description.strip().length(),1000));}
 public record UpdateAttachmentRequest(String description){}
 public record AttachmentResponse(UUID id,String filename,String contentType,long size,String description){static AttachmentResponse from(AgencyAttachment a){return new AttachmentResponse(a.getId(),a.getOriginalFilename(),a.getContentType(),a.getByteSize(),a.getDescription());}}
}
